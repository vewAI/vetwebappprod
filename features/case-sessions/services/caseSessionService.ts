...
// F6.5: professor-only group debrief report generated by AI from all
// completed attempts of the session.
export async function generateSessionReport(
  sessionId: string
): Promise<{ report: string; students: number }> {
  const res = await authFetch(
    `/api/case-sessions/${encodeURIComponent(sessionId)}/report`,
    { method: "POST" }
  );
  const body = await parseJson<{
    report?: string;
  ...
  return { report: body.report ?? "", students: body.students ?? 0 };
}
