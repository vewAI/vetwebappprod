...
export function CaseRagManager({ caseId }: Props) {
  ...
  const fetchKnowledge = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg =
          json.error || (res.status === 401 || res.status === 403 ? "Unauthorized to access knowledge audit." : "Failed to fetch knowledge");
        throw new Error(msg);
      }
  ...
  const handleDeleteSource = async (source: string) => {
    if (!confirm(`Are you sure you want to EXCLUDE all data from "${source}"? This will remove all extracted knowledge chunks for this reference.`))
      return;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          {items.length > 0 && (
            <button
              onClick={() => {
                const text = items
                  .map((i) => `[SOURCE: ${i.metadata?.source || "Internal"}] [CREATED: ${i.created_at}]\n${i.content}\n\n---`)
                  .join("\n");
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-700 truncate max-w-[120px]" title={name}>
                  {name}
                </span>
              </div>
            ...
            <div className="flex justify-between items-center">
              <span
                className={`text-[10px] px-1.5 py-0.5 rounded ${data.type === "PDF" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}
              >
                {data.type}
              </span>
              <button
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.length === 0 ? (
                  ...
                  <td colSpan={4} className="p-16 text-center">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      ...
                      <p className="text-sm max-w-sm mx-auto opacity-70 leading-relaxed">
                        If you recently uploaded a document, it may still be processing. Try clicking the{" "}
                        <RefreshCw className="inline w-3 h-3 mx-1" /> **Refresh** icon above to sync.
                      </p>
                ...
                filteredItems.map((item) => (
                  <tr
                    key={item.id}
                    className={`group border-l-4 transition-colors ${selectedIds.has(item.id) ? "bg-indigo-50/30 border-l-indigo-500" : "hover:bg-slate-50/50 border-l-transparent"}`}
                  >
                    ...
                    <td className="p-3 align-top">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${item.metadata?.source?.toLowerCase().includes(".pdf") ? "bg-orange-400" : "bg-blue-400"}`}
                          />
      ...
      <p className="text-[10px] text-slate-400 px-1">
        * Synthetic data is generated from the core case fields in the database. Deleting these won&apos;t affect the original case but will prevent
        the AI from seeing that specific summarized context.
      </p>
    </div>
  );
}
