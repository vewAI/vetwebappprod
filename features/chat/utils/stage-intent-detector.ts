...
function hasDirectionVerb(normalized: string) {
  return DIRECTION_WORDS.some((word) =>
    new RegExp(`\\b${escapeRegExp(word)}\\b`).test(normalized)
  );
}

function hasStageNumberReference(normalized: string, nextStageNumber: number) {
  return new RegExp(`\\b(stage|section|part|step)\\s*${nextStageNumber}\\b`).test(
    normalized
  );
}
...
function buildClosingRegex(currentStageTitle?: string) {
  ...
  return new RegExp(
    `${CLOSING_WORD_PATTERN.source}[^?.!]{0,60}\\b(?:${alternation})\\b`
  );
}
...
function hasPersonaKeywordMention(normalized: string, keywords: string[]) {
  ...
  return personas.some((keyword) =>
    new RegExp(`\\b${escapeRegExp(keyword)}\\b`).test(normalized)
  );
}
...
export function detectStageIntentLegacy(
  content: string,
  context: StageIntentContext
): StageIntentDetectionResult {
  ...
  const stageNumberMention = hasStageNumberReference(
    normalized,
    context.nextStageNumber
  );
  ...
  const stageTokenMatches = stageTokens.filter(
  ...
  const readyForStagePattern = stagePhrase
    ? new RegExp(
        `\\b(ready|time|start|begin)\\s+(for|to|with)\\s+${escapeRegExp(
          stagePhrase
        )}\\b`
      ).test(normalized)
    : false;
  ...
  if (mentionsNextStagePhrase && (directionVerb || politeCue)) {
    return positiveResult(
      "medium",
      "User referenced the next stage and included a direction verb/polite cue"
    );
  }
  if (stageNumberMention && (directionVerb || politeCue)) {
    return positiveResult(
      "medium",
      "User referenced the numbered next stage with a direction cue"
    );
  }
  ...
  if (mentionsStageKeywords && politeCue) {
    ...
    return positiveResult(
      "medium",
      "User referenced stage keywords alongside a polite cue"
    );
  }
...
export function detectStageIntentPhase3(
  content: string,
  context: StageIntentContext
): StageIntentDetectionResult {
  const normalized = normalizeContent(content ?? "");
  ...
  const handoffRequest =
    REQUEST_VERB_PATTERN.test(normalized) && personaMention;
  if (handoffRequest) heuristics.add("handoff-request");

  const isQuestion =
    QUESTION_WORD_PATTERN.test(normalized) || normalized.endsWith("?");
  if (
    isQuestion &&
    !handoffRequest &&
    !mentionsNextStagePhrase &&
    !stagePhraseRegex?.test(normalized)
  ) {
    return noMatch();
  }
  ...
  register(
    "stage-number-cue",
    stageNumberMention && (directionVerb || politeCue),
    "high",
    "User referenced the numbered next stage with a cue"
  );
  register(
    "next-stage-cue",
  ...
  register(
    ...
    handoffRequest,
    "high",
    "User requested to switch personas (e.g., talk to the owner)"
  );
  register(
    "direction-keywords",
  ...
  register(
    "polite-keywords",
    keywordHits >= 3 && politeCue,
    "medium",
    "User used several stage keywords alongside a polite cue"
  );
  register(
    "token-cluster",
